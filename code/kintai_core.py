...
３）押印有無については、以下の判断をしてください。
  ・実際の印鑑の陰影がある：〇
  ・印鑑の陰影の印刷がある：〇
  ・サインの筆跡がある：〇
  ・上記のどれもない場合：✖
{transport_block}画像ファイル名（アップロードファイル名）として、次の名前のみを記載してください（サーバー側のIDや別名は使わないこと）:
{display_file_name}
...
３）押印有無については、以下の判断をしてください。
  ...
  ・サインの筆跡がある：〇
  ・上記のどれもない場合：✖
{transport_block}ＰＤＦファイル名（アップロードファイル名）として、次の名前のみを記載してください（サーバー側のIDや別名は使わないこと）:
{display_file_name}
    ...
    return _auto_judgment_symbol(
        {
            ...
            "match_company": mc,
            "file_name": (row.get("file_name") or row.get(TARGET_FILE_NAME_COL) or "").strip(),
